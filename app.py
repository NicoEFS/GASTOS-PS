...
def estilo_tabla(df):
    ...
    """
    html = df.to_html(index=False, classes="tabla")
    return estilos + f'<div class="tabla-container">{html}</div>'
...
try:
    ...
    if opcion == "Generales":
        ...
        df_generales = df_def[df_def[col_patrimonio] == selected_patrimonio][[col_concepto, col_definicion]].sort_values(by=col_concepto)
        df_generales.columns = ["CONCEPTO", "DEFINICIÓN"]
        st.markdown(estilo_tabla(df_generales), unsafe_allow_html=True)

    elif opcion == "Contables":
        st.markdown("### 📘 Definiciones Contables")
        df_contables = df_def[df_def[col_patrimonio] == "PS-CONTABLE"][[col_concepto, col_definicion]].sort_values(by=col_concepto)
        if df_contables.empty:
            st.warning("⚠️ No hay definiciones contables registradas.")
        else:
            df_contables.columns = ["CONCEPTO", "DEFINICIÓN"]
            st.markdown(estilo_tabla(df_contables), unsafe_allow_html=True)
